        ...
        <section class="w-full flex flex-wrap">
            <div class="grid md:grid-cols-3 place-items-center gap-5">
                ...
                <div class="relative bg-white rounded-2xl overflow-hidden shadow-lg">
                    <img src="{{ Storage::url($ebooks->gambar) }}" class="w-full h-52 object-cover border-8 border-white rounded-t-2xl">
                    <div class="px-5 pb-5">
                        <button class="block text-xl font-bold text-teal-600 hover:text-teal-500 duration-150">
                            {{ $ebooks->nama }}
                        </button>
                        <p class="w-full flex flex-wrap mt-3 text-justify">{{ $ebooks->deskripsi }}</p>

                        <a href="{{ Storage::url($ebooks->download) }}" target="_blank">

                            <button
                            class="w-full py-2 flex justify-center items-center rounded-full text-teal-500 border border-teal-500 bg-white hover:text-white hover:bg-teal-500 duration-200 text-sm font-bold mt-5 cursor-pointer">Lihat Ebook Ini</button>
                        </a>
