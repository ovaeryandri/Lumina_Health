    ...
    <div class="w-full p-6 bg-gray-100 min-h-screen">
        <div class="w-full  bg-white rounded-lg shadow-md overflow-hidden">
            <div class="bg-teal-300 p-4">
                <h2 class="text-xl font-bold text-teal-800">Edit Blog Berita</h2>
            </div>

            <form method="POST" action="{{ route('blognews.update', $news->slug) }}" enctype="multipart/form-data"
                class="p-6">
                @csrf
                @method('PUT')
                    ...
                    <select name="id_blog_category" id="id_blog_category"
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}"
                                {{ $news->id_blog_category == $category->id ? 'selected' : '' }}>
                                {{ $category->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="content" id="content"
                        class="form-control w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300"
                        rows="5" required>{{ $news->content }}</textarea>
                </div>
                ...
                <div class="mb-3">
                    ...
                    @if ($news->gambar)
                        <img src="{{ asset('storage/' . $news->gambar) }}" class="w-32 h-32 object-cover mb-2"
                            alt="Gambar Program">
                    @endif
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="tanggal_upload" id="tanggal_upload"
                        class="form-control w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-300"
                        required value="{{ \Carbon\Carbon::parse($news->tanggal_upload)->format('Y-m-d') }}">
                </div>
